...
contract Flashloan is ICallee, DydxFlashloanBase {
    // The Direction of the swap execution
    enum Direction {
        KyberToUniswap,
        UniswapToKyber
    }

    // The arbitrage info including if the repay amount is sufficient
    struct ArbInfo {
        Direction direction;
        uint repayAmount;
    }

    // Emitting the event of the new arbitrage executed
    event NewArbitrage(Direction direction, uint profit, uint date);

    // kyber and uniswap are the two interfaces to execute the profitable flashloan with on-chain
    IKyberNetworkProxy kyber;
    IUniswapV2Router02 uniswap;

    // weth and dai are the pair of tokens used to execute the flashloan
    IWeth weth;
    IERC20 dai;

    // beneficiary is the address of which the profitable funs will be transfered to
    address beneficiary;

    // kyber_eth_address is the address of the on-chain liqudity pool operated  by kyber network
    address constant KYBER_ETH_ADDRESS =
        0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;

    // the contructor receive the addresses involved in the flashloans execution like liquidity providers, trading pairs and the address where the flashloans profits will be transfered to
    constructor(
        ...
        // assigning the interfaces to variables where you can interact with the interface more conviniently
